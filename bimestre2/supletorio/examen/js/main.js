...
jQuery.post("../es/php/index.html", {
    email: email
}, function(data, textStatus) {
    if (data == 0) {
        $("#mensaje").html("<div id='RegError'>El correo introducido has sido registrado anteriormente.</div>");
 
    } else if (data == 1) {
        $("#mensaje").html("<div id='RegSuccess'>Gracias por suscribirse.</div>");
 
        document.getElementById("email").value = "";
    } else {
        $("#mensaje").html("<div id='RegError'>Hubo un error y no se registraron los datos.</div>");
    }
});
